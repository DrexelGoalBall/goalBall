...
public class Wiimote {
	...
	public void update(string oscMessage, List<float> values, DateTime currentTime)
	{
		...
		else if(oscMessage == "ir")
		{
			...
			m_ir1RawX = values[0];
			m_ir1RawY = values[1];
			m_ir1RawSize = values[2];
		}
		else if(oscMessage == "ir/xys/2") {
			m_ir2RawX = values[0];
			m_ir2RawY = values[1];
			m_ir2RawSize = values[2];
		}
		else if(oscMessage == "ir/xys/3") {
			m_ir3RawX = values[0];
			m_ir3RawY = values[1];
			m_ir3RawSize = values[2];
		}
		else if(oscMessage == "ir/xys/4") {
			m_ir4RawX = values[0];
			m_ir4RawY = values[1];
			m_ir4RawSize = values[2];
		}
		// Wiimote Buttons
		...
		{
			m_buttonLeft = values[0];
		}
		else if(oscMessage == "button/Right")
		{
			m_buttonRight = values[0];
		}
		...
		{
			m_buttonDown = values[0];
		}
		else if(oscMessage == "button/Plus")
		{
			m_buttonPlus = values[0];
		}
		else if(oscMessage == "button/Minus")
		{
			m_buttonMinus = values[0];
		}
		else if(oscMessage == "button/1")
		{
			m_buttonOne = values[0];
		}
		else if(oscMessage == "button/2")
		{
			m_buttonTwo = values[0];
		}
		else if(oscMessage == "button/Home")
		{
			m_buttonHome = values[0];
		}
		else if(oscMessage == "motion/angles")
		{
			m_motionAnglePitch = values[0];
			m_motionAngleRoll = values[1];
			m_motionAngleYaw = values[2];
		}
		else if(oscMessage == "motion/velo")
		{
			m_motionVeloPitch = values[0];
			m_motionVeloRoll = values[1];
			m_motionVeloYaw = values[2];
		}
		// Balance Board analog
		else if(oscMessage == "balance")
		{
			m_balanceBL = values[0];
			m_balanceBR = values[1];
			m_balanceTL = values[2];
			m_balanceTR = values[3];
			m_balanceSum = values[4];
		}
		//Nunchuk
		...
		{
			m_nunchukJoyX = values[0];
			m_nunchukJoyY = values[1];
		}
		else if(oscMessage == "nunchuk/accel/pry")
		{
			m_nunchuckPitch = values[0];
			m_nunchukRoll = values[1];
			m_nunchukYaw = values[2];
			m_nunchukAccel = values[3];
		}
		else{}
	}
}
